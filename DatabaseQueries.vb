...
Module DatabaseQueries
    ...
    Public Function GetKanbanInfo(PN As String, Location As String)
        ...
        Dim query As String
        If PN <> Nothing Then
            query = "SELECT KANBAN_ID, REPL_STRATEGY, BIN_SIZE FROM SFM_RCH.KANBAN_PK_18_T WHERE PN = '" & PN & "' AND SUPPLY_AREA LIKE '%" & Location & "%' ORDER BY SUPPLY_AREA"
        Else
            query = "SELECT KANBAN_ID, REPL_STRATEGY, BIN_SIZE FROM SFM_RCH.KANBAN_PK_18_T WHERE SUPPLY_AREA LIKE '%" & Location & "%' ORDER BY SUPPLY_AREA"
        End If
        dal.RunQuery(query)
    ...
    Public Function GetBundleKanbanInfo(Location As String)
        Dim dal As New DALControl
        Dim query = "SELECT PN, KANBAN_ID, REPL_STRATEGY, BIN_SIZE, SUPPLY_AREA FROM SFM_RCH.KANBAN_PK_18_T WHERE SUPPLY_AREA LIKE '%" & Location & "%' ORDER BY PN, SUPPLY_AREA"
        dal.RunQuery(query)
